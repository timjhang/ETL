	...
	public void read_Collateral_File(String filePath, String fileTypeName, String batch_no, String exc_central_no,
		...
		try {
				...
				try {
					...
					while (strQueue.setTargetString() < strQueue.getByteListSize()) {
							...
							if (strQueue.getTotalByteLength() != 203) {
								...
								errWriter.addErrLog(new ETL_Bean_ErrorLog_Data(pfn, upload_no, "E",
										String.valueOf(rowCount), "行數bytes檢查", "非預期203:" + strQueue.getTotalByteLength()));
					...
					ETL_P_Log.write_Error_Log(batch_no, exc_central_no, exc_record_date, null, fileTypeName, 
							upload_no, "E", "0", "ETL_E_COLLATERAL程式處理", ex.getMessage(), null);
			...
			if (fileList.size() == 0) {
				...
				// 寫入Error_Log
				ETL_P_Log.write_Error_Log(batch_no, exc_central_no, exc_record_date, null, fileTypeName, 
						upload_no, "E", "0", "ETL_E_COLLATERAL程式處理", detail_exe_result_description, null);
