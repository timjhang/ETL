...
public class ETL_E_PARTY_ADDRESS {
	...
	public void read_Party_Address_File(String filePath, String fileTypeName, String batch_no, String exc_central_no,
		...
		try {
				...
				try {
					...
					while (strQueue.setTargetString() < strQueue.getByteListSize()) {
							...
							if (strQueue.getTotalByteLength() != 137) {
								...
								errWriter.addErrLog(new ETL_Bean_ErrorLog_Data(pfn, upload_no, "E",
										String.valueOf(rowCount), "行數bytes檢查", "非預期137:" + strQueue.getTotalByteLength()));
					...
					ETL_P_Log.write_Error_Log(batch_no, exc_central_no, exc_record_date, null, fileTypeName, 
							upload_no, "E", "0", "ETL_E_PARTY_ADDRESS程式處理", ex.getMessage(), null);
			...
			if (fileList.size() == 0) {
				...
				// 寫入Error_Log
				ETL_P_Log.write_Error_Log(batch_no, exc_central_no, exc_record_date, null, fileTypeName, 
						upload_no, "E", "0", "ETL_E_PARTY_ADDRESS程式處理", detail_exe_result_description, null);
	...
	public static void main(String[] argv) throws Exception {
		...
		String filePath = "D:\\PSC\\Projects\\AgriBank\\單元測試報告\\";
